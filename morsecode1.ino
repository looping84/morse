...
//**************************************************//
char stringToMorseCode[] = "BH1SOW PSE K";
...
int Spaces = dotLen * 3 - dotLen;     // length of the spaces between characters dot
...
{  // Loop through the string and get each character one at a time until the end is reached
  int strLen = sizeof(stringToMorseCode);
  for (int i = 0; i < strLen - 1; i++)
  {
	...
	if(i > 0 || i == strLen - 1) {//elementPause
		Serial.println(i);
		noTone(audio8);	   
  		delay(dotLen);
	}
...
{  
	...
	switch (tmpChar) {
	  ...
	  case '1':
		...
		break;
	  case '2':
	    MorseDot(); LightsOff(elemPause); MorseDot(); LightsOff(elemPause); 
	    MorseDash(); LightsOff(elemPause); MorseDash(); LightsOff(elemPause);
		MorseDash(); LightsOff(elemPause);
		break;
	  case '3':
	    MorseDot(); LightsOff(elemPause); MorseDot(); LightsOff(elemPause); 
	    MorseDot(); LightsOff(elemPause); MorseDash(); LightsOff(elemPause);
		MorseDash(); LightsOff(elemPause);
		break;
	   case '4':
	    MorseDot(); LightsOff(elemPause); MorseDot(); LightsOff(elemPause); 
	   ...
	   case '6':
	    MorseDash(); LightsOff(elemPause); MorseDot(); LightsOff(elemPause); 
	    MorseDot(); LightsOff(elemPause); MorseDot(); LightsOff(elemPause);
		MorseDot(); LightsOff(elemPause);
	    case '7':
	    MorseDash(); LightsOff(elemPause); MorseDash(); LightsOff(elemPause); 
